...
public class LeaveController {
    ...
    @ResponseBody
    @RequestMapping(value="/thisMonth", method= RequestMethod.GET,produces = "text/plain;charset=utf-8")
    public String thisMonth(HttpServletRequest request) throws Exception   //显示请假表中所有请假的信息
    {
        String pageString = request.getParameter("page");
        String limitString = request.getParameter("limit");
        Integer pageInteger = Integer.valueOf(pageString);
        Integer limitInteger = Integer.valueOf(limitString);
        System.out.println(pageInteger +" "+ limitInteger);
        List<Leave> listsList = iLeaveService.thisMonth(pageInteger,limitInteger);
        String[] colums = { "id", "name", "phone","data", "reason","result" };
        String data = ObjtoLayJson.ListtoJson(listsList, colums);
        System.out.println(data);
        return data;
